'''Write a Python program to replace the maximum value in a linked list with a given value.'''
...
import os, sys
# Ensure project root (two levels up) is on sys.path so `Modules` can be imported
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))

import Modules.LL as LL

def replace_max(ll, value):
    """Replace the maximum node's data in linked list `ll` with `value`."""
    temp = ll.head    # Start from the head of the linked list
    if temp is None:
        return

    max_node = temp   # Initialize max_node to the head node

    # Traverse the linked list to find the maximum node
    while temp is not None:
        if temp.data > max_node.data:
            max_node = temp
        temp = temp.next

    # Replace the maximum node's data with the given value
    max_node.data = value
...
print(L)  # Output: 3 -> 5 -> 2 -> 8 -> 1
replace_max(L,10)
print("Linked List after replacing max value with 10:")
print(L)  # Output: 3 -> 5 -> 2 -> 10 -> 1